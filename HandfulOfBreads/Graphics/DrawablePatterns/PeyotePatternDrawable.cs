...
{
    internal class PeyotePatternDrawable : IPatternDrawable
    {
        ...
        public bool _showSelectionOnly = false;
        ...
        private float GetOffsetY(int col)
        {
            int blockIndex = col / _drop;

            if (blockIndex % 2 != 0)
            {
                return _pixelSize / 2f;
            }
            else
            {
                return 0f;
        ...
        public void InitializeGrid(int rows, int columns, int pixelSize, IImage? fillImage = null, List<List<Color>> grid = null, int drop = 1)
        {
            _rows = rows;
            _columns = columns;
        ...
        public async void Draw(ICanvas canvas, RectF dirtyRect)
        {
            ...
            var cellsToDraw = _showSelectionOnly && SelectedCells.Any() ?
                SelectedCells :
                null;

            int minRow = 0, maxRow = _rows - 1, minCol = 0, maxCol = _columns - 1;
            if (cellsToDraw != null)
            {
                minRow = SelectedCells.Min(c => c.Row);
                maxRow = SelectedCells.Max(c => c.Row);
                minCol = SelectedCells.Min(c => c.Col);
                maxCol = SelectedCells.Max(c => c.Col);
            }
            ...
            for (int row = minRow; row <= maxRow; row++)
            {
                ...
                {
                    if (cellsToDraw != null && !SelectedCells.Contains((row, col)))
                        continue;

                    float offsetY = GetOffsetY(col);
                    float x = col * _pixelSize;
                    float y = row * _pixelSize + offsetY;

                    var color = _grid[row][col];
            ...
            if (!_showSelectionOnly && _selectionStartCell.HasValue && _selectionEndCell.HasValue)
            {
                var start = _selectionStartCell.Value;
                var end = _selectionEndCell.Value;

                int selMinRow = Math.Min(start.Row, end.Row);
                int selMaxRow = Math.Max(start.Row, end.Row);
                int selMinCol = Math.Min(start.Col, end.Col);
                int selMaxCol = Math.Max(start.Col, end.Col);
                ...
                {
                    float offsetY = GetOffsetY(cell.Col);
                    float x = cell.Col * _pixelSize;
                    float y = cell.Row * _pixelSize + offsetY;
            ...
            if (_isPasting && _pastingData != null && _pastePosition.HasValue)
            {
                ...
                for (int r = 0; r < _pasteRows; r++)
                {
                    for (int c = 0; c < _pasteCols; c++)
                    {
                        ...
                        if (gridRow >= 0 && gridRow < _rows && gridCol >= 0 && gridCol < _columns)
                        {
                            ...
                            if (color != Colors.Transparent)
                            {
                                float offsetY = GetOffsetY(gridCol);
                                float x = gridCol * _pixelSize;
                                float y = gridRow * _pixelSize + offsetY;
                ...
                if (_pastePosition.HasValue)
                {
                    int minPCol = _pastePosition.Value.Col;
                    int maxPCol = minPCol + _pasteCols - 1;
                    int minPRow = _pastePosition.Value.Row;
                    int maxPRow = minPRow + _pasteRows - 1;

                    float startX = minPCol * _pixelSize;
                    float endX = (maxPCol + 1) * _pixelSize;
                    float startY = minPRow * _pixelSize + GetOffsetY(minPCol);
                    float endY = (maxPRow + 1) * _pixelSize + GetOffsetY(maxPCol);

                    float pasteX = minPCol * _pixelSize;
                    float pasteY = minPRow * _pixelSize + GetOffsetY(minPCol);
                    float pasteWidth = _pasteCols * _pixelSize;
                    float pasteHeight = (_pasteRows) * _pixelSize + (_pasteCols % 2 == 0 ? 0 : _pixelSize / 2f);
            ...
            if (_selectedRow.HasValue)
            {
                ...
                for (int col = 0; col < _columns; col++)
                {
                    float offsetY = GetOffsetY(col);
                    float x = col * _pixelSize;
                    float cellY = _selectedRow.Value * _pixelSize + offsetY;

                    canvas.StrokeColor = _overlayColor;
                    canvas.StrokeSize = 5;
                    canvas.DrawRectangle(x, cellY, _pixelSize, _pixelSize);
                }
            }
        }
        ...
        public void TogglePixel(float x, float y)
        {
            ...
            int row = (int)((y - offsetY) / _pixelSize);
        ...
        public void HighlightRow(int? row)
        {
            ...
            {
                int newRow = (int)(_selectedRow.GetValueOrDefault(0) + row);
        ...
        {
            int maxWidth = _columns * _pixelSize;
            int maxHeight = _rows * _pixelSize + (int)(_pixelSize / 2f);

            using var surface = SKSurface.Create(new SKImageInfo(maxWidth, maxHeight));
