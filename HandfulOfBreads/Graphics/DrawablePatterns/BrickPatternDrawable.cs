...
namespace HandfulOfBreads.Graphics.DrawablePatterns
{
    internal class BrickPatternDrawable : IPatternDrawable
    {
        ...
        private float GetOffsetY(int col) => (col % 2 != 0) ? _pixelSize / 2f : 0f;

        public Color GetColorAt(int row, int col) => _grid[row][col];

        public void InitializeGrid(int rows, int columns, int pixelSize, IImage? fillImage = null, List<List<Color>> grid = null, int drop = 0)
        {
            _rows = rows;
            _columns = columns;
